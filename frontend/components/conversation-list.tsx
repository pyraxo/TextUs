    ...
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between p-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">You have {conversations.length} conversations</p>
        </div>
      ...
      <ScrollArea className="flex-1">
        {conversations.map((conversation) => (
          <Link
            ...
            className={cn(
              "flex flex-col gap-1 py-3 px-4 hover:bg-accent",
              pathname === `/conversations/${conversation.id}` && "bg-accent",
            )}
          >
