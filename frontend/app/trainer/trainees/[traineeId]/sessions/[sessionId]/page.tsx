"use client";

import { ConversationsInterface } from "@/components/conversations/conversations-interface";
import { CustomerInfo } from "@/components/conversations/customer-info";
import { Button } from "@/components/ui/button";
import { useSessionConversations } from "@/hooks/use-conversations";
import { WebSocketProvider } from "@/lib/providers/websocket-provider";
import { PanelRightClose, PanelRightOpen } from "lucide-react";
import { useParams } from "next/navigation";
import { useState } from "react";

export default function SessionDetailsPage() {
  const params = useParams();
  const traineeId = params?.traineeId as string;
  const sessionId = params?.sessionId as string;

  const [isSidebarOpen, setIsSidebarOpen] = useState(false);
  const [activeConversationId, setActiveConversationId] = useState<
    string | null
  >(null);

  // Fetch conversations for this session
  const {
    data: conversations,
    isLoading,
    error,
  } = useSessionConversations(traineeId, sessionId);

  if (isLoading) return <div>Loading session conversations...</div>;
  if (error) return <div>Error loading session conversations.</div>;
  if (!conversations)
    return <div>No conversations found for this session.</div>;

  // ConversationsInterface expects an activeSessionId, but in readOnly mode we just pass sessionId
  return (
    <WebSocketProvider readOnly>
      <div className="h-[calc(100vh-4rem)] w-full p-4">
        <div className="h-full rounded-xl border bg-background shadow-sm">
          <div className="h-full flex overflow-hidden relative">
            <div
              className={`flex-1 min-w-0 transition-all duration-300 ${
                isSidebarOpen ? "pr-[380px]" : ""
              }`}
            >
              <ConversationsInterface
                activeSessionId={sessionId}
                onConversationSelect={(conversationId) =>
                  setActiveConversationId(conversationId)
                }
                readOnly
              />
            </div>
            <Button
              variant="ghost"
              size="icon"
              className={`fixed z-50 transition-all duration-300 ${
                isSidebarOpen ? "right-[400px]" : "right-6"
              } top-[94px]`}
              onClick={() => setIsSidebarOpen(!isSidebarOpen)}
            >
              {isSidebarOpen ? (
                <PanelRightClose className="h-5 w-5" />
              ) : (
                <PanelRightOpen className="h-5 w-5" />
              )}
            </Button>
            <div
              className={`fixed top-[80px] right-[-400px] h-[calc(100vh-96px)] w-[380px] mr-[16px] border bg-background transition-transform duration-300 rounded-xl overflow-y-auto ${
                isSidebarOpen ? "translate-x-[-400px]" : "translate-x-0"
              }`}
            >
              <CustomerInfo conversationId={activeConversationId} />
            </div>
          </div>
        </div>
      </div>
    </WebSocketProvider>
  );
}
