...
async def validate_scenario_prerequisites(
    trainee_id: str, scenario_id: str, session: Session = Depends(get_session)
) -> Tuple[User, Scenario]:
    ...
    trainee = session.get(User, trainee_id)
    if not trainee:
...
async def complete_scenario(
    ...
    if active_conversations and status == SessionStatus.COMPLETED:
        raise HTTPException(
            status_code=400, detail="Cannot complete session with active conversations"
        )
    ...
    metrics = await get_session_metrics(session_id, session)
...
async def get_active_session(
    ...
    if results and results.start_timestamp <= timeout_threshold:
        ...
        await complete_scenario(results.id, SessionStatus.TIMED_OUT, session)
        return None
    ...
    for conversation in user_session.chat_conversations:
        ...
        for message in messages:
            ...
            if last_message_time and message.message_type != MessageType.USER:
                response_time = (message.timestamp - last_message_time).total_seconds()
